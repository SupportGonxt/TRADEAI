...
const CampaignBudget = ({ campaignId }) => {
  ...
  const utilization = data.totalBudget > 0 ? ((data.spentBudget || 0) / data.totalBudget) * 100 : 0;
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={3}><Card><CardContent><Typography variant="body2" color="text.secondary">Allocated</Typography><Typography variant="h6">{fmt(data.allocatedBudget)}</Typography></CardContent></Card></Grid>
        <Grid item xs={12} sm={3}><Card><CardContent><Typography variant="body2" color="text.secondary">Spent</Typography><Typography variant="h6">{fmt(data.spentBudget)}</Typography></CardContent></Card></Grid>
        <Grid item xs={12} sm={3}><Card><CardContent><Typography variant="body2" color="text.secondary">Remaining</Typography><Typography variant="h6">{fmt(data.remainingBudget)}</Typography></CardContent></Card></Grid>
